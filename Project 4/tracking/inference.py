    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                actual = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[actual] > 0:
                    allPossible[p] = emissionModel[actual] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
		    ...
		    positionDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))

		    for newPosition, probability in positionDistribution.items():
		    	updatedBeliefs[newPosition] = updatedBeliefs[newPosition] + probability * self.beliefs[oldPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while num < self.numParticles:
            particle = self.legalPositions[num % len(self.legalPositions)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
        	num = 0
        	while num < self.numParticles:
        		self.particles[num] = self.getJailPosition()
        		num += 1
        
        else:
            num = 0
            while num < self.numParticles:
            	i = self.particles[num]
            	beliefs[i] += 1
            	num += 1

            for belief in beliefs:
                actualDist = util.manhattanDistance(belief, pacmanPosition) 
                beliefs[belief] = beliefs[belief] * emissionModel[actualDist]
                
            if beliefs.totalCount() != 0: 
                num = 0
                while num < self.numParticles:
                    self.particles[num] = util.sample(beliefs)
                    num += 1
            else:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        updatedPositionDistribution = util.Counter()
        num = 0
        while num < self.numParticles:
            pos = self.particles[num]
            if updatedPositionDistribution[pos] == 0:
                ghostPos = self.setGhostPosition(gameState, pos)
                updatedPositionDistribution[pos] = self.getPositionDistribution(ghostPos)
            self.particles[num] = util.sample(updatedPositionDistribution[pos])
            num += 1
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles: 
            beliefDistribution[particle] = beliefDistribution[particle] + 1.0/self.numParticles  
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        num = 0
        while num < self.numParticles:
            particle = allPossibilities[num % len(allPossibilities)]
            self.particles.append(particle)
            num += 1
    ...
    def observeState(self, gameState):
        ...
        while num < self.numGhosts:
            if noisyDistances[num] == None:
                ghostsD.append(num)
            num += 1
        ...
        for particle in self.particles:
            ...
            num = 0
            while num < self.numGhosts:
                if num not in ghostsD:
                    model = emissionModels[num]
                    prob = prob*model[util.manhattanDistance(particle[num], pacmanPosition)]
                num += 1
        ...
        if counter.totalCount() != 0:
            self.beliefs.normalize()
            num = 0
            while num < len(self.particles):
                newPosition = util.sample(self.beliefs)
                self.particles[num] = newPosition
                num += 1
        else:
        	self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            num = 0
            while num < self.numGhosts:
                d = getPositionDistributionForGhost(setGhostPositions(gameState, oldPositions), num, self.ghostAgents[num])
                newParticle[num] = util.sample(d)
                num += 1
    ...
    def getBeliefDistribution(self):
        ...
        beliefs = util.Counter()
        for particle in self.particles:
            beliefs[particle] = beliefs[particle] + 1.0 / self.numParticles
        return beliefs
